...
skill 的意义是从随机系统中拧出确定性。可预测性 = agent 每次走同样的**过程**，而非产出同样的输出；其最强形式是确定性。语义工作中能抽取的确定性子任务（格式校验、字段扫描、路径检查、批量替换）尽量脚本化，不是什么都让模型生成。

## 2. 表达方式分层

基建/哲学级 skill 以「目标 + 可检查可穷尽的完成判据」为核心；编号步骤只在顺序本身是约束（先确认后动手、先 dry-run 后 write）或实际观察到抢跑时保留。业务 skill 必须有 SOP（阶段顺序、转交边界、停止条件）以保证过程确定性，但每一步都须过必要性审查，避免繁杂 SOP。
...
每个工作单元以判据收尾。判据必须可检查（能分辨 done/not-done）、可穷尽（“每条都核对”而非“给个清单”）。模糊判据招致 premature completion（抢跑，提前收工）。
...
默认不生成额外 output 文件；中间产物不作最终输出；必须生成文件时说明数量、路径、用途。
