...
分界线是可检验性，不是 skill 的层级：机械可判的写死——下沉脚本、schema、受控值表；判断性的给可证伪目标 + 规格，方法让路。机械化之前先问它拦不拦得住：脚本的能力边界见第 10 条，对语义质量与判断对错设机械门只会得到形式通过——门过了，东西还是空的。拦得住的，细度不设上限；拦不住的，不拿机械门伪装。编号步骤只在顺序本身是约束（先确认后动手、先 dry-run 后 write）或实际观察到抢跑时保留。
...
- skill 要求的输入先定位再读、只读所需片段。
- 每个产物——含中间产物——须说得出用途与服务对象，保持完成任务所需的最简形态；必须落盘的说明数量、路径、用途；说不出用途的不设。
- 机制同受此判据约束：未被要求的东西与遗漏的要求同级——多做了没人要的机制，与漏做了要做的事，都是偏离需求。为"稳健""正规""专业"增设的每一层——校验层、gate、schema、中间契约、抽象层——须说得出它服务哪个科研问题；说不出的即为多余，删。科研的交付物是结论、证据与方法，工程完备性只是手段：当一层机制的存在理由只能追溯到"这样更工程化"而非"这样才答得了这个问题"，它就是跑偏。
- 删规则文本时：重述使用点已说过的内容——社会证明、自我推销、末尾总结——是冗余，直接删；针对具体合理化给出反驳论证的文本承重，要折进使用点而非删除。删任何一条前先问：删掉之后哪个具体行为会退化？答不上来即冗余。
